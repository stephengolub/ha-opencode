...
`)}_handleTextSelection(){let e=window.getSelection();if(!e||e.isCollapsed)return;let i=e.toString().trim();if(!i)return;let t=this.querySelector(".history-body");if(!t)return;let s=e.anchorNode,n=e.focusNode;!s||!n||!t.contains(s)||!t.contains(n)||this._copyToClipboard(i)}async _loadMoreHistory(){if(!this._historyData||this._historyLoadingMore||!this._hass||!this._historySessionId)return;let e=this._historyData.total_count??this._historyData.messages.length;if(this._historyData.messages.length>=e){let n=this._historyData.messages.length;if(Math.max(0,n-this._historyVisibleCount)<=0)return;this._historyVisibleCount+=f.HISTORY_PAGE_SIZE,this._render();return}this._historyLoadingMore=!0,this._render();let s=this.querySelector(".history-body")?.scrollHeight||0;try{await this._hass.callService("opencode","get_history",{session_id:this._historySessionId,request_id:`loadmore_${Date.now()}`}),setTimeout(()=>{this._historyLoadingMore&&(this._historyLoadingMore=!1,this._render());let n=this.querySelector(".history-body");if(n&&s>0){let a=n.scrollHeight-s;n.scrollTop=a}},500)}catch(n){console.error("[opencode-card] Failed to load more history:",n),this._historyLoadingMore=!1,this._render()}}_renderPermissionModal(e){let i=!!e.permission_id,t=i?"":"disabled";return`
    ...
    `}_renderHistoryMessage(e){let i=e.role==="user",t=C(e.timestamp),s=e.parts.map(c=>{if(c.type==="text"&&c.content)return`<div class="history-text markdown-content">${this._renderMarkdown(c.content)}</div>`;if(c.type==="tool_call"){let h=c.tool_output||c.tool_error;return`
    ...
    `}_escapeHtml(e){let i=document.createElement("div");return i.textContent=e,i.innerHTML}_renderMarkdown(e){let i=[],t=e.replace(/```(\w*)\n([\s\S]*?)```/g,(h,d,g)=>{let m=i.length,v=this._escapeHtml(g.trimEnd()),_=d?` class="language-${d}"`:"";return i.push(`<pre><code${_}>${v}</code></pre>`),`\0CB${m}\0`}),s=[];t=t.replace(/`([^`]+)`/g,(h,d)=>{let g=s.length;return s.push(`<code>${this._escapeHtml(d)}</code>`),`\0IC${g}\0`});let n=t.split(`
`),o=[],a=!1,r="",l=!1;for(let h=0;h<n.length;h++){let d=n[h];if(/^(-{3,}|\*{3,}|_{3,})$/.test(d.trim())){a&&(o.push(r==="ul"?"</ul>":"</ol>"),a=!1),l&&(o.push("</blockquote>"),l=!1),o.push("<hr>");continue}let g=d.match(/^(#{1,6})\s+(.+)$/);if(g){a&&(o.push(r==="ul"?"</ul>":"</ol>"),a=!1),l&&(o.push("</blockquote>"),l=!1);let x=g[1].length,k=this._processInlineMarkdown(g[2]);o.push(`<h${x}>${k}</h${x}>`);continue}let m=d.match(/^>\s*(.*)$/);if(m){a&&(o.push(r==="ul"?"</ul>":"</ol>"),a=!1),l||(o.push("<blockquote>"),l=!0),o.push(this._processInlineMarkdown(m[1])+"<br>");continue}else l&&(o.push("</blockquote>"),l=!1);let v=d.match(/^(\s*)[-*+]\s+(.+)$/);if(v){(!a||r!=="ul")&&(a&&o.push(r==="ul"?"</ul>":"</ol>"),o.push("<ul>"),a=!0,r="ul"),o.push(`<li>${this._processInlineMarkdown(v[2])}</li>`);continue}let _=d.match(/^(\s*)\d+\.\s+(.+)$/);if(_){(!a||r!=="ol")&&(a&&o.push(r==="ul"?"</ul>":"</ol>"),o.push("<ol>"),a=!0,r="ol"),o.push(`<li>${this._processInlineMarkdown(_[2])}</li>`);continue}if(a&&d.trim()!==""&&(o.push(r==="ul"?"</ul>":"</ol>"),a=!1),d.trim()===""){a&&(o.push(r==="ul"?"</ul>":"</ol>"),a=!1),o.push("<br>");continue}o.push(`<p>${this._processInlineMarkdown(d)}</p>`)}a&&o.push(r==="ul"?"</ul>":"</ol>"),l&&o.push("</blockquote>");let c=o.join(`
`);return i.forEach((h,d)=>{c=c.replace(`\0CB${d}\0`,h)}),s.forEach((h,d)=>{c=c.replace(`\0IC${d}\0`,h)}),c}_processInlineMarkdown(e){let i=this._escapeHtml(e);return i=i.replace(/\[([^\]]+)\]\(([^)]+)\)/g,'<a href="$2" target="_blank" rel="noopener">$1</a>'),i=i.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),i=i.replace(/__([^_]+)__/g,"<strong>$1</strong>"),i=i.replace(/\*([^*]+)\*/g,"<em>$1</em>"),i=i.replace(/_([^_]+)_/g,"<em>$1</em>"),i=i.replace(/~~([^~]+)~~/g,"<del>$1</del>"),i}_renderEmpty(){return`
    ...
    `}_renderDevices(){let e=["idle","working","waiting_permission","waiting_input","error"],i=Array.from(this._devices.values()).filter(t=>!t.parentSessionId);return this._hideUnknown&&(i=i.filter(t=>{let s=t.entities.get("state")?.state??"";return e.includes(s)})),this._sortMode==="activity"?i.sort((t,s)=>{let n=t.entities.get("last_activity")?.state??"",o=s.entities.get("last_activity")?.state??"";if(!n&&!o)return 0;if(!n)return 1;if(!o)return-1;let a=new Date(n).getTime(),r=new Date(o).getTime();return isNaN(a)&&isNaN(r)?0:isNaN(a)?1:isNaN(r)?-1:r-a}):i.sort((t,s)=>{let n=t.deviceName.replace("OpenCode - ","").toLowerCase(),o=s.deviceName.replace("OpenCode - ","").toLowerCase();return n.localeCompare(o)}),i.map(t=>this._renderDevice(t)).join("")}_renderDetailView(e,i){let t=e.entities.get("state"),s=e.entities.get("session_title"),n=e.entities.get("model"),o=e.entities.get("current_tool"),a=e.entities.get("cost"),r=e.entities.get("tokens_input"),l=e.entities.get("tokens_output"),c=e.entities.get("last_activity"),h=t?.state??"unknown",d=$[h]||$.unknown,g=s?.state??"Unknown Session",m=n?.state??"unknown",v=o?.state??"none",_=a?.state??"0",x=r?.state??"0",k=l?.state??"0",w=c?.state??"",q=t?.attributes?.agent||null,y=t?.attributes?.current_agent||null,I=t?.attributes?.hostname||null,D=this._getChildSessions(e.sessionId),H=!!e.parentSessionId,O="";w&&(O=new Date(w).toLocaleTimeString());let T=this._getPermissionDetails(e),M="";if(T){let u=!!T.permission_id;M=`
      ...
      `);let S=this._getQuestionDetails(e),L="";if(S&&S.questions.length>0){let u=S.questions[0];L=`
        <div class="question-alert pinned clickable" data-device-id="${e.deviceId}">
          <ha-icon icon="mdi:comment-question"></ha-icon>
          <div class="question-details">
            <div class="question-title">${u.header||"Question"}</div>
            <div class="question-preview">${S.questions.length>1?`${S.questions.length} questions`:"Tap to answer"}</div>
          </div>
          <ha-icon icon="mdi:chevron-right" class="question-chevron"></ha-icon>
        </div>
      `}else h==="waiting_input"&&(L=`
        <div class="question-alert pinned clickable" data-device-id="${e.deviceId}">
          <ha-icon icon="mdi:comment-question"></ha-icon>
          <div class="question-details">
      ...
      <button class="back-button" data-action="back">
        ...
        <span>${H?"Parent Session":"Back"}</span>
      </button>
    `:"",G=H?`
    ...
    `:"",j=["working","waiting_permission","waiting_input"],R=D.filter(u=>j.includes(u.entities.get("state")?.state??"")),z=D.filter(u=>!j.includes(u.entities.get("state")?.state??"")),B=(u,b)=>{let F=u.entities.get("state")?.state??"unknown",N=$[F]||$.unknown,K=u.entities.get("session_title")?.state??"Unknown",U=u.entities.get("last_activity")?.state??"",V=U?C(U):null,J=u.entities.get("current_tool")?.state??"none";return`
        <div class="child-session-item clickable ${b?"active":""}" data-device-id="${u.deviceId}">
          ...
          <div class="child-session-info">
            <div class="child-session-title">${K}</div>
            ${b&&J!=="none"?`<div class="child-session-tool"><ha-icon icon="mdi:tools"></ha-icon> ${J}</div>`:""}
            ${!b&&V?`<div class="child-session-activity">${V.display}</div>`:""}
          </div>
          <ha-icon icon="mdi:chevron-right" class="child-session-chevron"></ha-icon>
        </div>
      `},A="";if(R.length>0){let u=R.map(b=>B(b,!0)).join("");A+=`
        <div class="child-sessions-section active-section">
          <div class="child-sessions-header active">
            <ha-icon icon="mdi:run-fast"></ha-icon>
            <span>Active Sub-agents (${R.length})</span>
          </div>
          <div class="child-sessions-list">
            ${u}
          </div>
        </div>
      `}if(z.length>0){let u=z.map(b=>B(b,!1)).join("");A+=`
        <div class="child-sessions-section">
          <div class="child-sessions-header">
            <ha-icon icon="mdi:source-branch"></ha-icon>
            <span>Sub-agent History (${z.length})</span>
          </div>
          <div class="child-sessions-list">
            ${u}
          </div>
      ...
      <div class="detail-view">
        ...
        <div class="detail-header">
          ...
          <div class="detail-project-info">
            ...
            ${I?`<div class="detail-hostname"><ha-icon icon="mdi:server"></ha-icon> ${I}</div>`:""}
          </div>
        </div>
        ...
        <div class="detail-info">
          ...
          <div class="detail-row">
            ...
            <span class="detail-value agent-badge">${q}${y&&y!==q?` <span class="sub-agent-indicator"><ha-icon icon="mdi:arrow-right"></ha-icon> ${y}</span>`:""}</span>
          </div>
          `:""}
          ${v!=="none"?`
      ...
      <div class="device-card clickable" data-device-id="${e.deviceId}">
        <div class="device-header">
          ...
          <div class="device-name-container">
            ...
            ${w?`<div class="device-activity" title="${w.tooltip}">${w.display}</div>`:""}
          </div>
        ...
        <div class="device-info">
          ...
          <div class="info-row">
            ...
            <span class="info-value sub-agent">${q}</span>
          </div>
          `:""}
    ...
    `}static getConfigElement(){return document.createElement("opencode-card-editor")}static getStubConfig(){return{title:"OpenCode Sessions"}}};f.HISTORY_PAGE_SIZE=10,f.HISTORY_REQUEST_TIMEOUT_MS=15e3;var Q=f,P=class extends HTMLElement{set hass(p){this._hass=p}setConfig(p){this._config=p,this._render()}_render(){this._config&&(this.innerHTML=`
      <style>
        .editor-row {
          display: flex;
          flex-direction: column;
